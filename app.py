...
def write_to_file(question, response, file_path="chat_history.txt"):
...
def handle_userinput(user_question):
    ...
    for i, message in enumerate(st.session_state.chat_history):
        if i % 2 == 0:
            ...
            write_to_file(user_message, bot_message)  # Write to file after each exchange
...
def main():
    ...
    local_directory = "PDFS"  # Replace with your directory path

    pdf_files = glob.glob(os.path.join(local_directory, "*.pdf"))
    if pdf_files:
        with st.spinner("Processing your question..."):
            pdf_docs = [open(file, 'rb') for file in pdf_files]
            # get pdf text
